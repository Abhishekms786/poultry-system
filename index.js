...
async function sendOTPEmail(email, otp, name='') {
  ...
  const data = JSON.stringify({
    ...
    htmlContent: '<div style="font-family:Arial,sans-serif;padding:24px;"><h2 style="color:#7B3F00;">Naati Koli Farm</h2><p>' + greeting + '</p><p>Your OTP is:</p><div style="background:#fdf3e0;border:2px solid #f5c842;border-radius:10px;padding:20px;text-align:center;"><span style="font-size:36px;font-weight:bold;color:#7B3F00;letter-spacing:8px;">' + otp + '</span></div><p>Valid for 10 minutes. Do not share.</p></div>'
  });
...
app.put('/api/owner/orders/:id', ownerAuth, async (req, res) => {
  try {
    ...
    const [order] = await pool.query('SELECT o.*,COALESCE(c.name,o.guest_name) as customer_name,COALESCE(c.phone,o.guest_phone) as customer_phone FROM orders o LEFT JOIN customers c ON o.customer_id=c.id WHERE o.id=?',[req.params.id]);
    res.json({ success: true, order: order[0] });
  } catch(err) { res.status(500).json({ error: err.message }); }
});
